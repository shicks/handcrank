...
import { VM } from './vm';
import { BIGINT, BOOLEAN, LEXICAL, NULL, NUMBER, OBJECT, STRING, SYMBOL, UNDEFINED } from './enums';
...
export abstract class Obj {
  abstract Prototype: Obj|null;
  abstract Extensible: boolean;
...
export function ValidateAndApplyPropertyDescriptor(
  ...
  if (O !== undefined) {
    ...
    //           field.
    const p = PropertyDescriptor({...unbrand(current)});
...
/** Uses defaults for all methods. */
export class OrdinaryObject extends Obj {
  Extensible = true;
  OwnProps = new Map<PropertyKey, PropertyDescriptor>();
  constructor(public Prototype: Obj|null) { super(); }
}
 ...
 */
export function OrdinaryObjectCreate(proto: Obj|null, additionalInternalSlotsList?: string[]) {
...
export abstract class Func extends Obj {
  ...
  abstract HomeObject: Obj|undefined;
  abstract ThisMode: LEXICAL|unknown;
